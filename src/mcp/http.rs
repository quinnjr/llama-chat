...
impl StreamableHttpTransport {
    ...
    async fn send_request(&mut self, method: &str, params: Option<serde_json::Value>) -> Result<JsonRpcResponse> {
        ...
        let resp = http_req.send().await.context("failed to send MCP request")?;
        ...
        if content_type.contains("text/event-stream") {
            ...
            let rpc_resp: JsonRpcResponse = serde_json::from_str(&body).context("failed to parse MCP response")?;
            if let Some(ref err) = rpc_resp.error {
                anyhow::bail!("MCP error {}: {}", err.code, err.message);
            }
            Ok(rpc_resp)
