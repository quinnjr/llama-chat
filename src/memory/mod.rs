...
pub use commands::{Command, parse as parse_command, save_ack};
...
impl MemoryService {
    pub async fn open(config: &AppConfig, project_dir: &Path) -> Result<Self, MemoryError> {
        ...
        if config.memory.embedding_model.is_empty() {
            return Err(MemoryError::Disabled(
                "memory.embedding_model not set".into(),
            ));
        }
    ...
    pub async fn list(
        ...
        use crate::memory::types::{Kind, Memory, Source};
        let store = match scope {
            Scope::Global => Arc::clone(&self.global),
            Scope::Project => Arc::clone(&self.project),
        };
    ...
    pub async fn recall(&self, query: &str) -> Result<Vec<RetrievedItem>, MemoryError> {
        ...
        tokio::task::spawn_blocking(move || -> Result<(), MemoryError> {
            ...
            for it in &returned {
                ...
                let store = match it.scope {
                    Scope::Global => &store_g,
                    Scope::Project => &store_p,
                };
                let conn = store.conn();
                let guard = conn.lock().expect("poisoned");
    ...
    pub async fn recover_orphans(
        ...
        let cutoff = now() - 3600;
        let orphans: Vec<i64> =
            tokio::task::spawn_blocking(move || -> Result<Vec<i64>, MemoryError> {
                let conn = store.conn();
                let guard = conn.lock().expect("poisoned");
                let mut stmt = guard
                    .prepare("SELECT id FROM sessions WHERE ended_at IS NULL AND started_at < ?")?;
                let ids = stmt
        ...
        for id in orphans {
            if self
                .extract_session(api, id, model_name.clone())
                .await
                .is_ok()
            {
                count += 1;
