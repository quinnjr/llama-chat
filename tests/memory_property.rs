...
fn chunker_losslessness() {
    ...
    let inputs = ["short one", "a b c d e f g h i j", &"word ".repeat(1200)];
    for input in inputs {
        ...
        for c in &chunks[1..] {
            ...
            for t in tail {
                reconstructed.push(t.to_string());
            }
        }
        let expected: Vec<String> = input.split_whitespace().map(|s| s.to_string()).collect();
        assert_eq!(
            reconstructed,
            expected,
            "input: {:?}",
